...
char * pack_coll_info(size_t *size_v, uint8_t coll_source, uint16_t userid1, uint16_t userid2, mpz_t b, mpz_t b2)
{
  /*
  [-]          [--]       [--]      [-][-]     [.?.]    [.?.]
 uint8        uint16     uint16     2*char      mpz_t    mpz_t
 coll_source  userid2     userid1  size_b/b2      b        b2
  */
  size_t size_vect,size_b,size_b2;
  size_t bsize_b,bsize_b2;
  char *vect;
  int i;

  bsize_b  = mpz_sizeinbase(b,2);
  bsize_b2 = mpz_sizeinbase(b2,2);

  size_b  = (unsigned char)((bsize_b-1)/8)+1;
  size_b2 = (unsigned char)((bsize_b2-1)/8)+1;

  *size_v = 2*sizeof(unsigned char);
  *size_v+= 2*sizeof(uint16_t);
  *size_v+= size_b + size_b2;

  vect = malloc(*size_v);

  mpz_export(vect+1+2*sizeof(unsigned char)+2*sizeof(uint16_t),       NULL,1,1,-1,0,b);
  mpz_export(vect+1+2*sizeof(unsigned char)+2*sizeof(uint16_t)+size_b,NULL,1,1,-1,0,b2);

  vect[0] = coll_source;

  for (i=0;i<sizeof(uint16_t);i++)
  {
    vect[i+1] = (userid1>>((sizeof(uint16_t)-1-i)*8))&0xff; // 0-3 1-4
  }
  for (i=0;i<sizeof(uint16_t);i++)
  {
    vect[i+1+sizeof(uint16_t)] = (userid2>>((sizeof(uint16_t)-1-i)*8))&0xff;
  }
  for (i=0;i<sizeof(unsigned char);i++)
  {
    vect[i+1+2*sizeof(uint16_t)] = 0xff&(unsigned char)size_b;
  }
  for (i=0;i<sizeof(unsigned char);i++)
  {
    vect[i+1+2*sizeof(uint16_t)+sizeof(unsigned char)] = 0xff&(unsigned char)size_b2;
  }
  return vect;
}

int unpack_coll_info(char *vect, uint8_t *coll_source, uint16_t *userid1, uint16_t *userid2, mpz_t b, mpz_t b2)
{
  /*
   [-]          [--]       [--]      [-][-]     [.?.]    [.?.]
  uint8        uint16     uint16     2*char      mpz_t    mpz_t
  coll_source  userid2     userid1  size_b/b2      b        b2
  */
  size_t size_b,size_b2;
  int i;
  i=1;
  //	size_b = (int)((nb_bits-1)/8)+1;
  //	size_x = (int)((nb_bits-1-trailing_bits)/8)+1;
  *coll_source = vect[0]&0xff;

  *userid1 = 0;
  for (i;i<1+sizeof(uint16_t);i++)
  {
    *userid1 <<= 8;
    *userid1 += (vect[i]&0xff);
  }
  *userid2 = 0;
  for (i;i<1+2*sizeof(uint16_t);i++)
  {
    *userid2 <<= 8;
    *userid2 += (vect[i]&0xff);
  }
  for (i;i<1+2*sizeof(uint16_t)+sizeof(unsigned char);i++)
  {
    size_b = (size_t)vect[i]&0xff;
  }
  for (i;i<1+2*sizeof(uint16_t)+2*sizeof(unsigned char);i++)
  {
    size_b2 = (size_t)vect[i]&0xff;
  }
  mpz_import(b, size_b ,1,1,-1,0,vect+i);
  i+=size_b;
  mpz_import(b2,size_b2,1,1,-1,0,vect+i);
  return i+size_b2;
}
...
long long int pcs_mu_run_shared_mem(int nb_threads, int world_rank,mpz_t x_res[__NB_USERS__], unsigned long long int times[__NB_USERS__],unsigned long int pts_per_users[__NB_USERS__])
{
  ...
  #pragma omp parallel private(userid1,R, b, r, xDist, xDist_no_pfx, xDist_str, trail_length,thread_num,tag,req,flagreq) shared(end_listener,end_stocker,end_searcher,userid_uptodate,end, trail_length_max, payload2,payload2_recv,payloads,payload_recv) num_threads(nb_threads+2)
  {
      ...
      {
        //char * payload;
        size_t size_vect_coll_info;
        int i,coll,payload_size,reqflag,nb_pts,payload_coll_info_size;
        MPI_Status status;
        USER_T userid2;
        mpz_t b2,x;
        char * payload_coll_info;
        ...
        while(!end_stocker)
        {
          ...
          if (coll)
          {
            if(DBG)printf("[%d] collision %d -> %d \n",status.MPI_SOURCE,userid1,userid2);
            //if(is_collision_mu(x, b, userid1, b2, userid2, trailling_bits))
            payload_coll_info = pack_coll_info(&size_vect_coll_info, (uint8_t) status.MPI_SOURCE, userid1, userid2, b, b2);
            //printf("[%d]packed! (%d) %d->%d\n",world_rank,status.MPI_SOURCE,userid1,userid2);
            MPI_Send(payload_coll_info, size_vect_coll_info, MPI_CHAR, 0, TAG_HANDLER, MPI_COMM_WORLD);
            //printf("[%d]sent!\n",world_rank);
            free(payload_coll_info);
          }
        }
      ...
      else if (thread_num==1) // listener
      {
        ...
        while(!end_listener)
        {
          MPI_Recv(&userid1, 1, MPI_INT, 0, TAG_NEXT_USER, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
          //printf("recv u%d\n",userid1);
          if (userid1>=__NB_USERS__)
          {
            ...
            MPI_Recv(&userid1,1,MPI_INT, MPI_ANY_SOURCE, TAG_END, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            #pragma omp critical (end)
            {
              end_stocker = 1;
            }
          ...
          {
            if(DBG)printf("[%d] u%d to u%d\n",world_rank,userid_uptodate,userid1);
            userid_uptodate = userid1;
          }
        }
      ...
      else if (thread_num==2 && world_rank==0) // user handler - extra listener thread for machine 0
      {
        int next_user = world_size;
        mpz_t b,b2;
        USER_T userid2;
        MPI_Status status;
        uint8_t *machine_statuses;
        USER_T *found_users;
        uint8_t coll_source;
        uint8_t payload_coll_info_size;
        char *payload_coll_info_recv;

        machine_statuses = malloc(world_size);
        found_users = malloc(sizeof(uint16_t) * __NB_USERS__);
        //found_users = malloc(__NB_USERS__);
        ...
        mpz_inits(b,b2,NULL);
        payload_coll_info_size = 1 + 2*sizeof(unsigned char)+2*sizeof(USER_T)+2*(int)((nb_bits-1)/8)+1;
        payload_coll_info_recv = malloc(payload_coll_info_size);

        while(!end_handler)
        {
          // recv TAG_HANDLER
          MPI_Recv(payload_coll_info_recv, payload_coll_info_size, MPI_CHAR, MPI_ANY_SOURCE, TAG_HANDLER, MPI_COMM_WORLD, &status);
          unpack_coll_info(payload_coll_info_recv, &coll_source, &userid1, &userid2, b, b2);
          //printf("unpacking %d %d %d\n",coll_source,userid1,userid2);

          if (!found_users[userid1]) // if userid1 is not already found
          {
            found_users[userid1] = 1;
            if(VERBOSE)printf("Found user %d\n",userid1);
            // todo : add collision info to database
            MPI_Send(&next_user, 1, MPI_INT, coll_source, TAG_NEXT_USER, MPI_COMM_WORLD);
            if(next_user==__NB_USERS__)
            {
              machine_statuses[coll_source] = 1;
              end_handler = 1;
              for (i=0;i<world_size;i++)
              {
                end_handler &= machine_statuses[i];
              }
              if(end_handler) // all machines are stopped, have to send TAG_END to every machine
              {

                for (i=0;i<world_size;i++)
                {
                  MPI_Send(&end_handler, 1, MPI_INT, i, TAG_END, MPI_COMM_WORLD); // sending 1 as end_handler has to be ==1
                }
              }
            }
            else
